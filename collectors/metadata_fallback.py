...
NON_RESEARCH_PATTERN = re.compile(
    ...
    r"abstracts\s+of\s+papers\s+presented|editors?[’'＊]?s?\s+notes?|"
    r"^\s*editorial(?:\s|:|$)|"
    ...
    r"excellence\s+in\s+refereeing\s+award|"
    r"fellows?(?:\s+of\b|:|\s*$)|^award\s+winning\s+theses?\s*$|"
    r"^\s*presidents?\s*$|prizes?\s+for\s+\d{4}|fischer\s+black\s+prize|"
    r"brattle\s+group|dimensional\s+fund\s+advisors|"
    r"annual\s+membership\s+meeting|report\s+of\s+the\s+est\b|"
    r"^introductory\s+essay\b|"
