...
更新发现与完整采集分离：系统每两小时通过 Crossref、官方 RSS 等低成本来源识别变化，只对确认有变化的期刊访问完整卷期与文章页。一次元数据异常不触发人工任务；连续失败达到阈值后才告警。任何新结果未通过质量门时，线上继续使用上一份已验证数据。
...
站点可以展示 `incomplete` 数据，但不得把它标为已完成；Composer 可以载入它供人工修补。目录顺序、备用来源、字段来源和完整 `quality_flags` 属于内部质量信息，统一在状态页与公共 API 披露，不在面向读者的卷期内容页显示技术性核验提示。
