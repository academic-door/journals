...
class R1AAERFreshnessWorkflowTests(unittest.TestCase):
    def test_scheduled_run_refreshes_live_authoritative_set_2025_2026(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("Refresh live authoritative discovery evidence", text)
        self.assertIn("--state data/backfill-state/field-2025-2026.json", text)
        self.assertIn("--journals AER,JPE,QE", text)
    ...
    def test_scheduled_live_refresh_forces_guarded_publish(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("force_publish_scheduled_live=true", text)
        self.assertIn('[ "$pending" != "[]" ] || [ "$force_publish_scheduled_live" = "true" ]', text)
